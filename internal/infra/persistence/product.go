...
func (productRepo *ProductRepositoryImpl) mappingProduct(products []*model.ProductModel) map[int64]*model.ProductModel {
...
func (productRepo *ProductRepositoryImpl) FindByIDsForUpdate(db *gorm.DB, productIDs []int64) (map[int64]*model.ProductModel, error) {
	var products []*model.ProductModel

	if err := db.Clauses(clause.Locking{Strength: "UPDATE"}).Where("id IN ?", productIDs).Preload("Category").Find(&products).Error; err != nil {
		return nil, err
	}

	return productRepo.mappingProduct(products), nil
}

func (productRepo *ProductRepositoryImpl) FindByIDs(db *gorm.DB, productIDs []int64) (map[int64]*model.ProductModel, error) {
	var products []*model.ProductModel
