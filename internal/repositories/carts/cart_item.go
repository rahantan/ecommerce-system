...
func (cartRepo *CartItemRepositoryImpl) GetAllCartItemByIDs(db *gorm.DB, cartIDs []int64, userID int64) ([]*models.CartItemModel, error) {
	var cartItems []*models.CartItemModel
...
func (cartRepo *CartItemRepositoryImpl) DeleteCartItemsByIDs(db *gorm.DB, cartIDs []int64, userID int64) error {
	...
	if err := db.Where("id IN ? AND user_id=?", cartIDs, userID).Delete(&models.CartItemModel{}).Error; err != nil {
